...
/*$studentId = 203;
 ...
 $studentName = utf8_decode($studentName);
		   ...
		   $sql = "SELECT * FROM usertracking where clientId = '$clientId' and courseId = '$courseId' and studentId = '$studentId'";
			...
			if ($userAvailable > 0) {	
				...
				if($totalUsers < $LicenseCount){
					...
					if($studentId != 'prabaharan.sachin@gmail.com') {
...
function getCourseStructure($courseId,  $courseTypeId) {
	...
	if ($result->num_rows > 0) {
		...
		if( $courseTypeId != 2) {
			$sql = "SELECT structure FROM coursedetails where courseId = '$courseId' and courseTypeId = 2";
			$result1 = $conn->query($sql);
